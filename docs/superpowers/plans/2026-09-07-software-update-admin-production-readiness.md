...
Every entry must be structurally valid; malformed entries fail the whole registry closed rather than being skipped. Structural validity requires the bounded array and exact entry keys/types, a canonical lowercase hex64 digest, a safe channel identifier string, nonempty unique valid `artifact_ids`, an exact valid UTC RFC3339-seconds timestamp, and boolean `enabled` and `revoked`; malformed shape, type, encoding, timestamp syntax, or duplicate keys is `SOFTWARE_UPDATE_RECORD_INVALID` (500). After validating `now`, an otherwise structurally valid entry is eligible for this beta endpoint only when its channel is exactly `beta`, expiry is strictly later than `now`, `enabled === true`, `revoked === false`, artifact scope includes the requested Core, and digest matches. Hash the decoded 32 token bytes with SHA-256, decode each configured digest to 32 bytes, and compare equal-length bytes with `crypto.timingSafeEqual`. Missing/malformed registry is a server configuration error; missing header is 401; syntactically invalid header or no eligible matching entry—including unknown digest, expired/equal, disabled, revoked, channel-mismatched, or out-of-scope—is `DISTRIBUTION_CAPABILITY_INVALID` (403). Raw token, digest, registry, service key, storage identity, and signed URL never appear in errors or logs.
...
- [ ] **B2 — Add exact-registry structural tests.** Cover absent/empty/non-array/oversized registry; empty array; non-object entry; duplicate keys; missing/extra fields; uppercase/malformed digest; non-string or unsafe channel identifier; empty/duplicate/oversized/invalid artifact scope; malformed/non-UTC/fractional expiry syntax; invalid clock; non-boolean `enabled`/`revoked`; and one malformed entry beside one valid entry. Every structurally malformed registry must fail closed with `SOFTWARE_UPDATE_RECORD_INVALID` (500), not skip.
- [ ] **B3 — Add authorization eligibility tests.** Use deterministic synthetic 32-byte tokens and injected time. Prove exact token + beta + explicit Core artifact scope + future expiry + `enabled:true` + `revoked:false` succeeds; unknown digest, expired or equal expiry, disabled, revoked, channel mismatch, and out-of-scope are structurally valid but ineligible and fail with `DISTRIBUTION_CAPABILITY_INVALID` (403). Spy on `crypto.timingSafeEqual` through a focused injectable/internal seam if direct observation is otherwise brittle; assert byte comparison and ensure raw token/digest/sentinels are absent from all safe error representations.
...
- [ ] **B6 — Implement strict authorization.** In `server/software-update.mjs`, add canonical token decoding; exact bounded structural registry parsing with duplicate-key rejection, safe channel identifiers, strict timestamp syntax, and boolean state fields; then separately enforce beta channel, future expiry, enabled/non-revoked state, exact scope, and digest as authorization eligibility returning 403 when unmet. Compare SHA-256 bytes through `crypto.timingSafeEqual`. Parse no registry for Launcher; parse and authorize before any Core signer call.
